...
class WeatherSpec extends Specification {
    ...
    def "Weather could be created from valid values"() {
        given:
        def weather = new Weather(temperature, wind, 100)
        when:
    ...
    def "Temperature could not be null"() {
        given:
        def weather = new Weather(null, wind, 100)
        when:
        def errors = validator.validate(weather)
        then:
        errors.size() == 1
    ...
    def "Wind could not be null"() {
        given:
        def weather = new Weather(temperature, null, 100)
        when:
        def errors = validator.validate(weather)
        then:
        errors.size() == 1
    ...
    def "Clouds coverage could not be null"() {
        given:
        def weather = new Weather(temperature, wind, null)
        when:
        def errors = validator.validate(weather)
        then:
        errors.size() == 1
    ...
    def "Clouds coverage could not be less then 0"() {
        given:
        def weather = new Weather(temperature, wind, -1)
        when:
        def errors = validator.validate(weather)
        then:
        errors.size() == 1
    ...
    def "Clouds coverage could not be bigger then 100"() {
        given:
        def weather = new Weather(temperature, wind, 150)
        when:
        def errors = validator.validate(weather)
        then:
        errors.size() == 1
